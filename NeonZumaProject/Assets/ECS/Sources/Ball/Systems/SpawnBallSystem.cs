        ...
        for(int i = 0; i < entities.Count; i++) {
            ...
            for(int j = 0; j < lastBalls.Length; j++) {
                if(lastBalls[j].trackId.value == entities[i].trackId.value) {
                    distance = lastBalls[j].positionBall.value - _contexts.game.levelConfig.value.offsetBetweenBalls;
            ...
            entityBall.AddPositionBall(distance);
            entityBall.AddViewBall(ball);
